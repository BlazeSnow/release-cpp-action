...
打上 `v1.0.0` 标签推送后，六个平台会分别编译 `src` 目录下的程序，并把产物上传到该 tag 的 Release：
...
- `myapp-v1.0.0-windows-arm64.exe`

> macOS 的 `-latest` 标签只有 arm64，x64 需用 `macos-26-intel`（`-large` 后缀为付费 larger runner）；Linux arm64 免费标签为 `ubuntu-24.04-arm`；Windows arm64 使用 `windows-11-arm`（自带 gcc / CMake / Visual Studio）。
