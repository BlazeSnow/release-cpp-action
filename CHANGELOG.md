...
1. 新增输入 `cxx-standard`：直接编译模式可指定 C++ 标准（编号如 `20`，或完整 `-std` 值如 `gnu++20`），默认仍为 `17`；CMake 模式不受影响，由 `CMakeLists.txt` 决定
2. 直接编译模式向下支持 C：编译 `*.c`，新增输入 `c-standard`（编号或完整 `-std` 值，缺省为编译器默认）；目录同时含 C++ 源文件时，`.c` 由 C 编译器单独编译后一并链接
