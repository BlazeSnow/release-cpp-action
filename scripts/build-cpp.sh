...
case "$(uname -s)" in
	...
	Darwin) os="macos" ;;
	*)
		echo "::error::不支持的操作系统: $(uname -s)（Windows 请使用 build-cpp.ps1）"
		exit 1
		;;
esac
...
if [ -f "$base_dir/CMakeLists.txt" ]; then
	...
	if [ ! -f "$built" ]; then
		...
		while IFS= read -r -d '' f; do
			candidates+=("$f")
		done < <(find "$build_dir" -type f -name "$name" -not -path "*/CMakeFiles/*" -print0)
	...
	while IFS= read -r -d '' f; do
		sources+=("$f")
	done < <(find "$base_dir" -maxdepth 1 -type f \( -name '*.cpp' -o -name '*.cc' -o -name '*.cxx' \) -print0)
	...
	# 空数组安全展开：macOS 自带的 bash 3.2 在 set -u 下不允许 "${arr[@]}"
	link_flags=()
	if [ "$os" = "linux" ]; then
		# 静态链接 C++ 运行库，产物在旧系统上也能运行；macOS 不支持该选项
		link_flags=(-static-libstdc++ -static-libgcc)
	fi

	"$compiler" -std=c++17 -O2 -o "$output" ${link_flags[@]+"${link_flags[@]}"} "${sources[@]}"
fi
